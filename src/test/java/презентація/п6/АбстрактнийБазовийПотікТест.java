...
public class АбстрактнийБазовийПотікТест {
    ...
    public void дляКожногоТест() {
        ...
        презентація.п5.ФабрикаПотоків.потік(циферки).дляКожного(новийСписок::add);
    ...
    @Test
    public void знайтиПершийРезультатЄ() {
        Optional<Integer> першийЕлементПотоку = ФабрикаПотоків.потік(циферки).знайтиПерший();
        assertTrue(першийЕлементПотоку.isPresent());
    }

    @Test
    public void знайтиПершийРезультатуНемає() {
        Optional<Integer> елементПотокуБільшийЗа42 = ФабрикаПотоків.потік(циферки)
                .фільтр(число -> число > 42)
                .знайтиПерший();
        assertFalse(елементПотокуБільшийЗа42.isPresent());
    }
}