...
public interface Потік<T> extends Iterator<T> {
    ...
    /**
     * Повертає {@link Optional} з першим елементом потоку або порожній {@link Optional}, якщо потік пустий.
     * Метод є {@code short-circuit} термінальним методом
     *
     * @return {@link Optional}, який описує перший елемент потоку
     */
    Optional<T> знайтиПерший();
}
